...
import pro.sihao.jarvis.domain.model.PipeCatTransportState
...
class PipeCatIntegrationTest {
    ...
    fun `test PipeCat service initialization`() {
        ...
        assertEquals("Initial connection state should be DISCONNECTED", 
                   PipeCatTransportState.DISCONNECTED, initialState.transportState)
        assertFalse("Bot should not be connected initially", initialState.botIsConnected)
    ...
    fun `test PipeCat connection state flow`() {
        ...
        assertEquals("Initial transport state should be DISCONNECTED", 
                   PipeCatTransportState.DISCONNECTED, states[0].transportState)
        assertFalse("Bot should not be connected initially", states[0].botIsConnected)
